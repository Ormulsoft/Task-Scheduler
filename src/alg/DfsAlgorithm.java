...
public class DfsAlgorithm implements Algorithm {
	...
	public void PartialSol(int indexOfVerticeList, int numProcessors, int cost, List<List<Integer>> list, ScheduleGrph input){
		...
		for(int i : verticeList){
			for(int j = 0; j < numProcessors; j++){
				...
				if(visited.containsKey(i) && visited.containsValue(j)){	
					...
					// If parents are all visited so that dependencies are all checked
