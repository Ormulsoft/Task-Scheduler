...
/*
 * Simple sequential algorithm to generate a valid but not necessarily optimal solution.
 * Intended as a placeholder, replaced by Astar algorithm. VERY SLOW
 */
	...
	public ScheduleGrph runAlg(ScheduleGrph input, int numCores, int numProcessors) {
		...
		// Keep a list of all tasks that havent been given a start time or processor
		...
		while (!unassignedTasks.isEmpty()) {
			...
			if (!taskReady) {
				// If the task still has unassigned dependencies, skip it for now
				...
				//  find earliest possible start and the processor that gives this
				int[] slot = findEarliestStart(task);
				
				// Assign properties according to earliest start time/processor, remove task from list of unassigned
	...
	/**
	 * Get the earliest start time available for this task (based on trial and error)
	...
	private int[] findEarliestStart(int task) {
		...
		// If no dependencies, assign to whichever processor has the earliest available slot
		if (deps.isEmpty()) {
			...
			// otherwise, try the task on each processor and find out the earliest time on each that it can start
			// based on processor earliest free time and time required for dependencies to transfer/finish
